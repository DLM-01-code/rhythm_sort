      ...
      renameTrack: async (id, newName) => {
        ...
        try {
          ...
          if (renameResult.ok) {
            ...
            // ✅ Записываем новое имя во все основные текстовые теги
            if (typeof (api as any).writeFullTags === 'function') {
              try {
                await (api as any).writeFullTags(newPath, {
                  title: newName,
                  artist: newName,
                  album: newName,
                  year: newName,
